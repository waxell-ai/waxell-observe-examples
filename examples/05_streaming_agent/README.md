...
Side-by-side OpenAI and Anthropic streaming with chunk-by-chunk accumulation and token counting.
...
# Custom query
python examples/05_streaming_agent/streaming_agent.py --dry-run --query "Your question here"

# Trigger policy enforcement (intentionally exceed limits)
python examples/05_streaming_agent/streaming_agent.py --policy-triggers
```
...
3. For real LLM calls (instead of mock), set your provider key: `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`
