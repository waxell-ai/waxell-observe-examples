...
Calls OpenAI, Anthropic, and Groq through LiteLLM's unified API with auto-instrumentation comparing responses across providers.
...
# With real API calls
python examples/11_litellm_agent/litellm_agent.py

# Custom query
python examples/11_litellm_agent/litellm_agent.py --dry-run --query "Your question here"

# Trigger policy enforcement (intentionally exceed limits)
python examples/11_litellm_agent/litellm_agent.py --policy-triggers
```
...
3. For real LLM calls (instead of mock), set your provider key: `LITELLM_API_KEY`
