...
PR review pipeline using Anthropic: diff parsing, parallel static analysis (linter, tests, security scanner), reasoning chain, and review generation with retry.
...
# With real API calls
python examples/09_code_review_agent/code_review_agent.py

# Custom query
python examples/09_code_review_agent/code_review_agent.py --dry-run --query "Your question here"

# Trigger policy enforcement (intentionally exceed limits)
python examples/09_code_review_agent/code_review_agent.py --policy-triggers
```
...
3. For real LLM calls (instead of mock), set your provider key: `ANTHROPIC_API_KEY`
