...
3-step content analysis pipeline (classify, extract entities, summarize) using the Anthropic SDK with auto-instrumentation.
...
# Custom query
python examples/01_anthropic_agent/anthropic_agent.py --dry-run --query "Your question here"

# Trigger policy enforcement (intentionally exceed limits)
python examples/01_anthropic_agent/anthropic_agent.py --policy-triggers
```
