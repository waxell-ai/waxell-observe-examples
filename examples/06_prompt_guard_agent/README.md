...
PII, credential, and prompt injection detection in 3 modes: block (raise error), warn (log and continue), redact (sanitize placeholders).
...
# With real API calls
python examples/06_prompt_guard_agent/prompt_guard_agent.py

# Custom query
python examples/06_prompt_guard_agent/prompt_guard_agent.py --dry-run --query "Your question here"

# Trigger policy enforcement (intentionally exceed limits)
python examples/06_prompt_guard_agent/prompt_guard_agent.py --policy-triggers
```
