# SDK Enrichment Showcase

Demonstrates all enrichment features: @observe decorator, scores (numeric, boolean, categorical), tags, metadata, and top-level convenience functions.
...
# With real API calls
python examples/10_enrichment_agent/enrichment_agent.py

# Custom query
python examples/10_enrichment_agent/enrichment_agent.py --dry-run --query "Your question here"

# Trigger policy enforcement (intentionally exceed limits)
python examples/10_enrichment_agent/enrichment_agent.py --policy-triggers
```
...
| Provider key(s) | For real calls | `OPENAI_API_KEY` |
